...
def test_adaptive_task_on_settle_triggers_loss_stop(tmp_path):
    ...
    assert update.get("active") is False
    assert update.get("ended", {}).get("status") == "loss_stop"
    assert ctx.state.runtime.get("current_task_run_id", "") == ""


def test_adaptive_task_freeze_blocks_auto_trigger(tmp_path):
    ctx = _make_user(tmp_path, user_name="freeze_user", user_id=10004)
    ctx.state.history = [0, 1] * 40
    ...
    assert blocked.get("reason") == "frozen"
